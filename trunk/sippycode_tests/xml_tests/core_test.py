...
import sippycode.xml.core as xml_core
...
class Child(xml_core.XmlElement):
  _qname = ('{http://example.com/1}child', '{http://example.com/2}child')


class Foo(xml_core.XmlElement):
  _qname = 'foo'


class Example(xml_core.XmlElement):
...
class Inner(xml_core.XmlElement):
  _qname = '{http://example.com/xml/1}inner'
  my_x = 'x'


class Outer(xml_core.XmlElement):
...
class XmlElementTest(unittest.TestCase):
  ...
  def testGetQName(self):
    class Unversioned(xml_core.XmlElement):
      _qname = '{http://example.com}foo'

    class Versioned(xml_core.XmlElement):
      _qname = ('{http://example.com/1}foo', '{http://example.com/2}foo') 

    self.assert_(
        xml_core._get_qname(Unversioned, 1) == '{http://example.com}foo')
    self.assert_(
        xml_core._get_qname(Unversioned, 2) == '{http://example.com}foo')
    self.assert_(
        xml_core._get_qname(Versioned, 1) == '{http://example.com/1}foo')
    self.assert_(
        xml_core._get_qname(Versioned, 2) == '{http://example.com/2}foo')
  ...
  def testGetElements(self):
    ...
    e.foos.append(Foo())
    e.foos[1].text = 'foo2'
    e._other_elements.append(xml_core.XmlElement())
  ...
  def testParseBasicXmlElement(self):
    element = xml_core.xml_element_from_string(SAMPLE_XML, 
        xml_core.XmlElement)
  ...
  def testSchemaParse(self):
    outer = xml_core.xml_element_from_string(SAMPLE_XML, Outer)
...
class UtilityFunctionTest(unittest.TestCase):
  ...
  def testMatchQnames(self):
    self.assert_(xml_core._qname_matches(
...
def suite():
  return unittest.TestSuite((unittest.makeSuite(UtilityFunctionTest,'test'),
                             unittest.makeSuite(XmlElementTest, 'test')))
